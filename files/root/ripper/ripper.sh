...
SOURCE_DRIVE=${SOURCE_DRIVE:-"/dev/sr1"}
...
do
    ...
    INFO=$"`makemkvcon -r --cache=1 info disc:9999 | egrep ^${DEVICE},`"
    
    # check INFO for optical disk
    EMPTY=`echo $INFO | grep -o "${DEVICE},0,999,0,\""`
    OPEN=`echo $INFO | grep -o "${DEVICE},1,999,0,\""`
    LOADING=`echo $INFO | grep -o "${DEVICE},3,999,0,\""`
    BD1=`echo $INFO | grep -o "${DEVICE},2,999,12,\""`
    BD2=`echo $INFO | grep -o "${DEVICE},2,999,28,\""`
    DVD=`echo $INFO | grep -o "${DEVICE},2,999,1,\""`
    CD1=`echo $INFO | grep -o "${DEVICE},2,999,0,\""`
    ...
    # if [ $EMPTY = "${DEVICE},0,999,0,"' ]; then
    #  echo "$(date "+%d.%m.%Y %T") : No Disc"; &>/dev/null
    # fi
    if [ "$OPEN" = "${DEVICE},1,999,0,\"" ]; then
        echo "$(date "+%d.%m.%Y %T") : Disk tray open"
    fi
    if [ "$LOADING" = "${DEVICE},3,999,0,\"" ]; then
        echo "$(date "+%d.%m.%Y %T") : Disc still loading"
    fi
    
    if [ "$BD1" = "${DEVICE},2,999,12,\"" ] || [ "$BD2" = "${DEVICE},2,999,28,\"" ]; then
    ...
    if [ "$CD1" = "${DEVICE},2,999,0,"${DEVICE} ]; then
        if [ "$CD2" = '","","'$DRIVE'"' ]; then
